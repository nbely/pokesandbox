...
/** Collections that are managed by the seed / export workflow. */
const SUPPORTED_COLLECTIONS = ['users', 'servers', 'regions'] as const;
...
async function exportCollection(
  ...
  console.info(
    `  Exported ${docs.length} document(s) → ${path.relative(process.cwd(), outputPath)}`
  );
}
